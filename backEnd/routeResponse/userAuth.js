...
exports.login = async (req, res) => {
  try {
    ...
    // console.log(req.body)
    let loginData = await UserModel.findOne({ email }).select("+password");
...
//     console.log(process.env.GOOGLE_CLIENT_ID, 'Google id')
...
//     console.log(email, name, picture, 'name email picture')
//     if (!payload.email_verified) {
//         throw new Error("Email not verified by Google");
//       }
//     // checking if there user exist ?
...
exports.googleLogin = async (req, res, next) => {
  try {
    ...
    // Verifing token using Google's API directly
    ...
    if (email_verified !== "true" && payload.email_verified !== true) {
      throw new Error("Email not verified by google");
    }
...
exports.resetPassword = async (req, res, next) => {
  ...
  // creating hash token
  const resetPasswordToeknHash = crypto
    .createHash("sha256")

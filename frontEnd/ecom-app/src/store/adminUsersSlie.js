...
export const getAllUsers = () => {
  return async function getAllUsersThunk(dispatch, getState) {
    ...
    try {
      const resData = await fetch(`${apiUrl}/api/v1/admin/allusers`, {
        credentials: "include",
      });
...
export const deleteUser = (id) => {
  return async function deleteUserThunk(dispatch, getState) {
    dispatch(setStatus(STATUSES.LOADING));
    try {
      const resData = await fetch(`${apiUrl}/api/v1/admin/user/delete/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await resData.json();
...
export const getUserDetail = (id) => {
  return async function getUserDetailThunk(dispatch, getState) {
    dispatch(setStatus(STATUSES.LOADING));
    try {
      const resData = await fetch(`${apiUrl}/api/v1/admin/user/details/${id}`, {
        credentials: "include",
      });
      const data = await resData.json();
...
export const updateUserRole = (id, name, email, phone, role) => {
  return async function updateUserRoleThunk(dispatch, getState) {
    ...
    try {
      const resData = await fetch(`${apiUrl}/api/v1/admin/user/update/${id}`, {
        method: "PUT",
        headers: {

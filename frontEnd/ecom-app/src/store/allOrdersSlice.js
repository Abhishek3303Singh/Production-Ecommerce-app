...
export const getAdminOrders = () => {
  return async function getAdminOrdersThunk(dispatch, getState) {
    ...
    try {
      ...
      const resData = await fetch(`${apiUrl}/api/v1/admin/orders/all`, {
        method: "get",
        credentials: "include",
        // body: formData,
      });
...
export const deleteOrder = (id) => {
  return async function deleteOrderThunk(dispatch, getState) {
    try {
      const resData = await fetch(`${apiUrl}/api/v1/admin/order/delete/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await resData.json();
...
export const setUpdateOrderStatus = (status, id) => {
  return async function setUpdateOrderStatusThunk(dispatch, getState) {
    ...
    try {
      ...
      const resData = await fetch(`${apiUrl}/api/v1/admin/order/status/${id}`, {
        method: "PUT",
        headers: {
